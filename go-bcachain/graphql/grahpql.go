...
import (
	...
	ethereum "github.com/bcachain/go-bcachain"
...
// `Log` objects.
func runFilter(ctx context.Context, be *bcac.EthAPIBackend, filter *filters.Filter) ([]*Log, error) {
...
func NewHandler(be *bcac.EthAPIBackend) (http.Handler, error) {
	q := Resolver{be}
...
type Service struct {
	...
	backend  *bcac.EthAPIBackend // The backend that queries will operate onn.
...
func NewService(backend *bcac.EthAPIBackend, endpoint string, cors, vhosts []string, timeouts rpc.HTTPTimeouts) (*Service, error) {
	return &Service{
		endpoint: endpoint,
		cors:     cors,
		vhosts:   vhosts,
