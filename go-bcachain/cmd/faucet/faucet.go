...
type faucet struct {
	...
	client *bcacclient.Client  // Client connection to the Ethereum chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	if stats != "" {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			...
			ctx.Service(&serv)
			return bcacstats.New(stats, nil, serv)
		}); err != nil {
			return nil, err
		}
	}
	...
	client := bcacclient.NewClient(api)
...
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	for {
		...
		default:
			err = errors.New("Something funky happened, please open an issue at https://github.com/bcachain/go-bcachain/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
