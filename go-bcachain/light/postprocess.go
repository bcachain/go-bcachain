...
import (
	...
	"github.com/bcachain/go-bcachain/common/bitutil"
...
func GetChtRoot(db bcacdb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
...
// Note that sectionIdx is specified according to LES/1 CHT section size.
func StoreChtRoot(db bcacdb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
type ChtIndexerBackend struct {
	diskdb, trieTable    bcacdb.Database
	odr                  OdrBackend
...
func NewChtIndexer(db bcacdb.Database, odr OdrBackend, size, confirms uint64) *core.ChainIndexer {
...
func GetBloomTrieRoot(db bcacdb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func StoreBloomTrieRoot(db bcacdb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func NewBloomTrieIndexer(db bcacdb.Database, odr OdrBackend, parentSize, size uint64) *core.ChainIndexer {
