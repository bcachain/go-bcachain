...
// package bcac implements the Ethereum protocol.
package bcac
...
import (
	...
	"github.com/bcachain/go-bcachain/bcac/filters"
	...
	"github.com/bcachain/go-bcachain/core/bloombits"
	...
	"github.com/bcachain/go-bcachain/internal/ethapi"
	"github.com/bcachain/go-bcachain/log"
	"github.com/bcachain/go-bcachain/miner"
...
type Ethereum struct {
	...
	// DB interfaces
	chainDb bcacdb.Database // Block chain database
...
func CreateConsensusEngine(ctx *node.ServiceContext, chainConfig *params.ChainConfig, config *ethash.Config, notify []string, noverify bool, db bcacdb.Database) consensus.Engine {
	// If proof-of-authority is requested, set it up
	if chainConfig.PoBC != nil {
		return pobc.New(chainConfig.PoBC, db)
	}
...
func (s *Ethereum) shouldPreserve(block *types.Block) bool {
	// The reason we need to disable the self-reorg preserving for pobc
...
func (s *Ethereum) StartMining(threads int) error {
	...
	if !s.IsMining() {
		...
		if pobc, ok := s.engine.(*pobc.PoBC); ok {
			wallet, err := s.accountManager.Find(accounts.Account{Address: eb})
			if wallet == nil || err != nil {
				log.Error("Etherbase account unavailable locally", "err", err)
				return fmt.Errorf("signer missing: %v", err)
			}
			pobc.Authorize(eb, wallet.SignData)
		}
...
func (s *Ethereum) ChainDb() bcacdb.Database           { return s.chainDb }
