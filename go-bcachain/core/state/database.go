...
type Trie interface {
	...
	Prove(key []byte, fromLevel uint, proofDb bcacdb.Writer) error
}
...
func NewDatabase(db bcacdb.Database) Database {
...
func NewDatabaseWithCache(db bcacdb.Database, cache int) Database {
...
func (m cachedTrie) Prove(key []byte, fromLevel uint, proofDb bcacdb.Writer) error {
