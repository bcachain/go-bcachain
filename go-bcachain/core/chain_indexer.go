...
type ChainIndexer struct {
	chainDb  bcacdb.Database      // Chain database to index the data from
	indexDb  bcacdb.Database      // Prefixed table-view of the db to write index metadata into
...
func NewChainIndexer(chainDb bcacdb.Database, indexDb bcacdb.Database, backend ChainIndexerBackend, section, confirm uint64, throttling time.Duration, kind string) *ChainIndexer {
	c := &ChainIndexer{
		chainDb:     chainDb,
		indexDb:     indexDb,
		backend:     backend,
