...
func NewLesServer(bcac *bcac.Ethereum, config *bcac.Config) (*LesServer, error) {
	quitSync := make(chan struct{})
	pm, err := NewProtocolManager(
		bcac.BlockChain().Config(),
	...
	for i, pv := range AdvertiseProtocolVersions {
		lesTopics[i] = lesTopic(bcac.BlockChain().Genesis().Hash(), pv)
	}

	srv := &LesServer{
		lesCommons: lesCommons{
			config:           config,
			chainDb:          bcac.ChainDb(),
			iConfig:          light.DefaultServerIndexerConfig,
			chtIndexer:       light.NewChtIndexer(bcac.ChainDb(), nil, params.CHTFrequencyServer, params.HelperTrieProcessConfirmations),
			bloomTrieIndexer: light.NewBloomTrieIndexer(bcac.ChainDb(), nil, params.BloomBitsBlocks, params.BloomTrieFrequency),
			protocolManager:  pm,
		},
		costTracker:  newCostTracker(bcac.ChainDb(), config),
		quitSync:     quitSync,
		lesTopics:    lesTopics,
	...
	srv.chtIndexer.Start(bcac.BlockChain())
	return srv, nil
}
